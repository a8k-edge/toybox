...
try:  # packages included in task Docker container
    ...
    sns.set_style("dark")
    COLORS = {"purple": "#98A1E1", "light-purple": "#DADEFB", "gold": "#F0C054"}
except ImportError:
...
class DataTableProcessor(FlowSpec):
    ...
    def plot_char_word_histogram(
        ...
        ax[0] = _df.char_count.plot.hist(
            bins=self.n_bins, color=COLORS["purple"], ax=ax[0]
        )
        ax[1] = _df.word_count.plot.hist(
            bins=self.n_bins, color=COLORS["light-purple"], ax=ax[1]
        )
    ...
    def plot_tld_count(self):
        ...
        self.processed_df.groupby("tld").count()["index"].sort_values(
            ascending=False
        ).plot.bar(ax=ax, color=COLORS["gold"])
    ...
    def start(self):
        ...
        if not run.successful:
            ...
            current.card.append(
                Markdown(f"""# Processing data table from run {run.id}""")
            )
            ...
            _df["tld"] = _df["page_url"].apply(
                lambda url: "https://" + tldextract.extract(url).fqdn
            )
            ...
            print(
                "Filtered dataframe from shape {} to shape {}.".format(
                    df.shape, _df.shape
                )
            )
    ...
    def end(self):
        ...
        print(
            "The {} run {} has ended, with a dataframe of shape: {}".format(
                current.flow_name, current.run_id, self.processed_df.shape
            )
        )
        print(
            f"""
