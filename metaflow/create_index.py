...
class PineconeVectorIndexer(FlowSpec):
    ...
    @kubernetes(
        image="registry.hub.docker.com/eddieob/rag:pinecone-vector-indexer-mf-task"
    )
    @step
    def start(self):
    ...
    def create_index(self):
        ...
        try:
            ...
            print(
                "Issue creating Pinecone index. If you are on the free plan, it is likely you have already met the index quota."
            )
    ...
    def end(self):
        ...
        index = pc.Index(self.index_name)
        matches = index.query(
            vector=self._test_search_vector.tolist(), top_k=K, include_metadata=True
        )
        ...
        print(
            "\n\n Flow is done, check for results in the {} index at https://app.pinecone.io/.".format(
                self.index_name
            )
        )
